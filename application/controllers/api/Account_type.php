...
class Account_type extends REST_Controller {
    ...
    {   
        ...
        if ($validate['code'] == 200) {
            ...
            $data = $this->Account_type_model->get_all($validate['data'], $filter, $order, $limit, $offset);
    ...
    {   
        ...
        if ($validate['code'] == 200) {
            $data = $this->Account_type_model->get_list($validate['data']);
    ...
    private function do_create()
    {
        ...
        if ($validate['code'] == 200) {
            $data = $this->Account_type_model->save($validate['data'], $this->input_fields());
    ...
    private function do_delete($id)
    {
        ...
        if ($validate['code'] == 200) {
            $data = $this->Account_type_model->delete($validate['data'], array('id' => $id));
