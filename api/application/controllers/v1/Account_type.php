...
class Account_type extends REST_Controller {
    ...
    private function do_get_all() 
    {
        ...
        if ($validated) {
            ...
            $data = $this->Account_type_model->get_all($filter, $order, $limit, $offset);
    ...
    {        
        ...
        if ($validated) {
            ...
            $list = $this->Account_type_model->get_list($order);
    ...
    private function do_create() 
    {
        ...
        if ($validated) {
            $id = $this->get_post("id");
            $account_group_id = $this->get_post("account_group_id");
            ...
            if (!$account_group_id || intval($account_group_id) <= 0) {
                $error[] = "Insert/Update Data Failed, `account_group_id` Expected INT";
            }
