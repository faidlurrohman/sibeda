...
class Account_object_detail_sub extends REST_Controller {
    ...
    /**
     * GET ALL ACCOUNT OBJECT DETAIL SUB
     */
    private function do_get_all() 
    {
        ...
        if ($validated) {
            ...
            $data = $this->Account_object_detail_sub_model->get_all($filter, $order, $limit, $offset);
    ...
    /**
     * GET ALL ACCOUNT OBJECT DETAIL SUB ACTIVE
     */
    ...
    {        
        ...
        if ($validated) {
            ...
            $list = $this->Account_object_detail_sub_model->get_list($order);
    ...
    private function do_create() 
    {
        ...
        if ($validated) {
            $id = $this->get_post("id");
            $account_object_detail_id = $this->get_post("account_object_detail_id");
            ...
            if (!$account_object_detail_id || intval($account_object_detail_id) <= 0) {
                $error[] = "Insert/Update Data Failed, `account_object_detail_id` Expected INT";
            }
            ...
            $save = $this->Account_object_detail_sub_model->save($params);

            if ($save['code'] == 200) {
                $this->response($save);
                // insert log
                $this->Log_model->log("account_object_detail_sub", $mode, $params, $validated->username);
            } else {
    ...
    /**
     * DISABLE ACCOUNT OBJECT DETAIL SUB
     * @param id [INT]
     */
    ...
    private function do_remove($id)
    {
        ...
        if ($validated) {
            ...
            $remove = $this->Account_object_detail_sub_model->delete(intval($id));

            if ($remove['code'] == 200) {
                $this->response($remove);
                // insert log
                $this->Log_model->log("account_object_detail_sub", "D", array("id" => intval($id)), $validated->username);
            } else {
    ...
    private function input_fields()
    {
        return array(
            ...
            "account_object_detail_id" => intval($this->get_post("account_object_detail_id")),
            "label" => $this->get_post("label"),
            "remark" => $this->get_post("remark"),
            "mode" => $this->get_post("mode"),
        );
