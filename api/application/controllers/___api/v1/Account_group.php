...
class Account_group extends REST_Controller {
    ...
    private function do_get_all() 
    {
        ...
        if ($validated) {
            ...
            $data = $this->Account_group_model->get_all($filter, $order, $limit, $offset);
    ...
    {        
        ...
        if ($validated) {
            ...
            $list = $this->Account_group_model->get_list($order);
    ...
    private function do_create() 
    {
        ...
        if ($validated) {
            $id = $this->get_post("id");
            $account_base_id = $this->get_post("account_base_id");
            ...
            if (!$account_base_id || intval($account_base_id) <= 0) {
                $error[] = "Insert/Update Data Failed, `account_base_id` Expected INT";
            }
    ...
    private function input_fields()
    {
        return array(
            ...
            "account_base_id" => intval($this->get_post("account_base_id")),
            "label" => $this->get_post("label"),
            "remark" => $this->get_post("remark"),
            "mode" => $this->get_post("mode"),
        );
